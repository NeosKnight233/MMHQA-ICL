...
# CAPTION_FILE = 'mmqa_captions_llava.json' # 这里检索的时候用的是原 Caption 信息, 不能更换, 否则出错
...
class Question_Image_Match_Classifier(object):
    ...
    def load_retrieve_info(self):
        df_qc = pd.read_csv(os.path.join(ROOT_DIR, "utils", "qc_mmqa_dev.csv"))
        ...
        df_qimc = pd.read_csv(os.path.join(ROOT_DIR, "utils", "qimc_mmqa_dev.csv"))
