...
caption_map = get_caption_map(os.path.join(ROOT_DIR, 'utils', 'mmqa_captions_llava.json'))
...
def get_caption_by_id(id):
    try:
        caption = caption_map[id]
    except:
        caption = ""
    return caption
...
class RetrieverDataset(Dataset):
    def __init__(self, tokenizer, split) -> None:
        ...
        for data in tqdm(self.data):
            ...
            # print(question_ids)
            # print(self.tokenizer.tokenize(data['question']))
            # print(self.tokenizer.convert_tokens_to_ids(tokenizer.sep_token))
            # print(self.tokenizer.convert_tokens_to_ids(tokenizer.cls_token))
            # exit(0)
...
if __name__ == '__main__':
    ...
    dataset = RetrieverDataset(tokenizer ,'dev')
    dataloader = DataLoader(dataset, batch_size=1, collate_fn=lambda x: collate(x, tokenizer, MAX_LEN))
